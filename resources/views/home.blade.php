...
@section('content')
    ...
    <div class="container-fluid mt-5">
        ...
        <h4>Mis grupos</h4>
        <div class="row mt-3">
            @if(!is_null(Auth::user()->circles))
                @foreach(Auth::user()->circles as $circle)
                    <div class="col-md-3 ml-5">
                        <div class="card " style="width: 18rem;">
                            <div class="card-body">
                                <h3 class="card-title">{{$circle->name}}</h3>
                                @foreach($circle->friends as $friend)
                                    {{$friend->friend_id}}
                                @endforeach
                            </div>
                            <div class="text-center mb-3">
                                <a href="{{ route('groups.edit', $circle->id) }}" class="btn btn-warning"><i class="fas fa-wrench"></i>
                                </a>
                                <a href="{{ route('group.destroy', $circle->id) }}" class="btn btn-danger"><i class="fas fa-trash-alt"></i>
                                </a>
                            </div>
